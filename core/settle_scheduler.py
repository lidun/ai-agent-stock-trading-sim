...
from core import quotes_tencent, settle_day
...
class EodSettleTrigger:
    ...
    def settle_once(self, now: datetime | None = None) -> dict:
        ...
        if not settle_day.pending_any(self.state, dstr):
        ...
        errors = [a for a in accounts if a.get("error")]
        if not errors:
            self._done_dates.add(dstr)
            self._audit("trade.eod_settle_auto", "ok",
                        f"{dstr} 结算完成，账户 {len(accounts)} 个")
            return {"date": dstr, "status": "settled", "accounts": accounts}
